...
/**
 * Diese Klasse implementiert abstrakte Rahmendaten als {@code stack-frame} einer Funktion.<br>
 * Ein Rahmendaten stellt eine unveränderliche Liste von Parameterwerten sowie ein konstantes Kontextobjekt zur Verfügung. Über die {@link #size() Anzahl der
 * Parameterwerte} hinaus, können von der Methode {@link #get(int)} auch zusätzliche Parameterwerte der übergeordneten Rahmendaten bereitgestellt werden.<br>
 * Die Methode {@link #get(int)} muss für einen gegebenen Index immer den gleichen Wert liefern bzw. immer eine Ausnahme auslösen. Analoges gilt für die
 ...
 */
public abstract class FEMFrame implements Items<FEMValue>, Iterable<FEMValue>, UseToString, Script.ScriptFormatterInput {
	...
	@SuppressWarnings ("javadoc")
	static final class InvokeArray extends FEMArray {
		...
		InvokeArray(final FEMFrame frame, final FEMFunction[] params) {
			super(params.length);
			this.__frame = frame;
			this.__params = params;
	...
	static final class InvokeFrame extends FEMFrame {
		...
		final InvokeArray __params;

		InvokeFrame(final FEMFrame parent, final InvokeArray params, final FEMContext context) {
			super(parent, context);
			this.__params = params;
		}
	...
	/**
	 * Dieses Feld speichert den leeren Rahmendaten, der keine Parameterwerte bereitstellt, das Kontextobjekt {@link FEMContext#EMPTY} verwendet und sich selbst
	...
	/**
	 * Diese Methode gibt neue Rahmendaten zurück, welche keine {@link #params() zugesicherten Parameterwerte} besitzt, das {@link #context() Kontextobjekt}
	 * dieses Objekts übernimmt und dieses Objekt als {@link #parent() übergeordnete Rahmendaten} nutzt.
	 * 
	 * @return neue Rahmendaten.
	 */
	...
	/**
	 * Diese Methode gibt neue Rahmendaten zurück, welche die gegebenen {@link #params() zugesicherten Parameterwerte} besitzt, das {@link #context()
	 * Kontextobjekt} dieses Objekts übernimmt und dieses Objekt als {@link #parent() übergeordnete Rahmendaten} nutzt.
	 * 
	...
	/**
	 * Diese Methode gibt neue Rahmendaten zurück, welche die gegebenen Parameterfunktionen zur Berechnung der {@link #params() zugesicherten Parameterwerte}
	 * verwendet, das {@link #context() Kontextobjekt} dieses Objekts übernimmt und dieses Objekt als {@link #parent() übergeordnete Rahmendaten} nutzt.
	 * <p>
	 * Die zugesicherten Parameterwerte werden mit Hilfe dieser Rahmendaten und der gegebener Parameterfunktionen ermittelt. Eine Parameterfunktion wird zur
	 * Ermittlung eines Parameterwerts einmalig mit diesen Rahmendaten {@link FEMFunction#invoke(FEMFrame) ausgewertet}. Genauer entspricht der {@code index}-te
	 ...
	 * Die über {@link #params()} bereitgestellte Wertliste der erzeugten Rahmendaten liefert die noch nicht über {@link #get(int)} ermittelten Parameterwerte als
	 * {@link FEM.VirtualValue}.
	 * 
	...
	public final FEMFrame newFrame(final FEMFunction... params) throws NullPointerException {
		return new InvokeFrame(this, new InvokeArray(this, params), this.__context);
	}

	/**
	 * Diese Methode gibt diese Rahmendaten mit den gegebenen {@link #params() zugesicherten Parameterwerten} zurück.<br>
	 * Sie ist eine Abkürzung für {@code this.parent().newFrame(params)}.
	 * 
	...
	public final FEMFrame withParams(final FEMArray params) throws NullPointerException {
		return this.__parent.newFrame(params);
	}
	...
	/**
	 * Diese Methode gibt diese Rahmendaten mit den gegebenen Parameterfunktionen zur Berechnung der {@link #params() zugesicherten Parameterwerte} zurück.<br>
	 * Sie ist eine Abkürzung für {@code this.parent().newFrame(params)}.
	 * 
	...
	/**
	 * Diese Methode gibt diese Rahmendaten mit dem gegebenen {@link #context() Kontextobjekt} zurück.
	 * 
	 ...
	 * Über die {@link #size() Anzahl der Parameterwerte} hinaus, können auch zusätzliche Parameterwerte der {@link #parent() übergeordneten Rahmendaten}
	 * bereitgestellt werden. Genauer wird für einen {@code index >= this.size()} der Parameterwert {@code this.parent().get(index - this.size())} der
	 * übergeordneten Rahmendaten geliefert.
	 * 
	 * @param index Index.
