...
/** Diese Klasse ergänzt einen {@link MappedBuffer2} um Methoden zur Auslagerung typisierter Speicherbereiche in eine Datei mit der nachfolgend spezifizierten
 * Dateistruktur.
 * <p>
...
public class MappedBuffer_TODO extends MappedBuffer {

	/** Diese Klasse implementiert eine auf 16 Byte ausgerichteten Speicherbereich innerhalb eines {@link MappedBuffer_TODO} mit {@link #hashCode()} und
	...
	private static final class Range {
		...
		final MappedBuffer_TODO store;

		Range(final MappedBuffer_TODO store, final int addr, final int size) {
			this.store = store;
			this.addr = addr;
			this.size = size;
		...
		public boolean equals(final Object object) {
			...
			if (!(object instanceof MappedBuffer_TODO.Range)) return false;
			final MappedBuffer_TODO.Range that = (MappedBuffer_TODO.Range)object;
			if (this.hash != that.hash) return false;
			final MappedBuffer_TODO store = this.store;
	...
	public MappedBuffer_TODO(final File file, final boolean readonly) throws IOException {
		super(file, readonly);
		if (!readonly) {
