...
class FTData {
	...
	public FTData(final String filePath, final long testSize) {
	...
	public boolean equals(final Object object) {
		...
		if (!(object instanceof FTData)) return false;
		final var that = (FTData)object;
		if (Objects.equals(this.filePath, that.filePath) || (this.testSize != that.testSize)) return true;
		try (var thisChannel = FTData.openChannel(this.filePath)) {
			try (var thatChannel = FTData.openChannel(that.filePath)) {
				final var testSize = this.testSize;
				final var fileSize = thisChannel.size();
				if (fileSize <= (2 * testSize)) return FTData.equalsChannel(thisChannel, thatChannel, fileSize);
				if (!FTData.equalsChannel(thisChannel, thatChannel, testSize)) return false;
				return FTData.equalsChannel(thisChannel.position(fileSize - testSize), thatChannel.position(fileSize - testSize), testSize);
			}
	...
	static void digestChannel(final MessageDigest target, final FileChannel source, final long length) throws IOException {
		final var bufSize = FTData.BUFFER_SIZE;
		final var hashBuffer = FTData.BUFFER_HASH;
		for (var remSize = length; remSize > 0; remSize -= bufSize) {
			final var done = FTData.readChannel(source, hashBuffer.limit((int)Math.min(remSize, bufSize)).position(0));
	...
	static boolean equalsChannel(final FileChannel thisChannel, final FileChannel thatChannel, final long length) throws IOException {
		final var bufSize = FTData.BUFFER_SIZE;
		final var bufThis = FTData.BUFFER_THIS;
		final var bufThat = FTData.BUFFER_THAT;
		for (long remSize = length; remSize > 0; remSize -= bufSize) {
			final int remLimit = (int)Math.min(remSize, bufSize);
			final var thisLast = FTData.readChannel(thisChannel, bufThis.limit(remLimit).position(0));
			final var thatLast = FTData.readChannel(thatChannel, bufThat.limit(remLimit).position(0));
	...
	private static final ByteBuffer BUFFER_THIS = ByteBuffer.allocateDirect(FTData.BUFFER_SIZE);

	private static final ByteBuffer BUFFER_THAT = ByteBuffer.allocateDirect(FTData.BUFFER_SIZE);

	private static final ByteBuffer BUFFER_HASH = ByteBuffer.allocateDirect(FTData.BUFFER_SIZE);
