...
class __Parser {
	...
	public static class Token implements Comparable<Token> {
		...
		/** Diese Methode liefert die Eingabe des {@link __Parser}, auf die sich die dieser Abschnitt bezieht. */
	...
	/** Diese Klasse implementiert die Zeichenkette als Eingabe eines {@link __Parser}.
	 *
	...
	public static class Source {
		...
		public static Source from(char[] chars) throws NullPointerException {
			return Source.from(chars, 0, chars.length);
		}
		...
		/** Diese Methode liefert eine Zeichenkette mit der gegebenen Zeichen. */
		public static Source from(String string) throws NullPointerException {
			return Source.from(string, 0, string.length());
		}
		...
		public boolean equals(Object object) {
			...
			char[] chars1 = this.chars, chars2 = that.chars;
			for (int i = 0, offset1 = this.offset, offset2 = that.offset; i < length; i++) {
				if (chars1[i + offset1] != chars2[i + offset2]) return false;
			}
			return true;
		}
